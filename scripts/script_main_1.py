...
def send_multi_address(sender:address, recievers:address, sending_contract:contract, token:contract, amount):
    len_rec = len(recievers)
    totalAmount = amount*len_rec
    sendingContractAddress = sending_contract.functions.contractAddress().call()
    tokenAddress = token.functions.address
    token_approve(sender, token, sendingContractAddress, int(totalAmount))
    
    construct_txn = sending_contract.functions.send_token_multiple(tokenAddress, int(amount), recievers).buildTransaction({
    'from': sender,
    ...
    token_approve(sender, token, sendingContractAddress, int(0))
    return token.functions.balanceOf(sender).call(), token.functions.balanceOf(sendingContractAddress).call() 
...
if __name__ == "__main__": 
    ...
    # token_approve(accounts[0], DAI, w3.toChecksumAddress(contract_address), out1)
    # print("Allowance: %s \n"%(get_allowance(DAI, accounts[0], w3.toChecksumAddress(contract_address))/1e18))

    # # send_token_multiple(address _token, uint _amount, address[] calldata _recievers)
    ...
    balance_sender, balance_contract = send_multi_address(accounts[0], recievers, sending_contract, DAI, int(10e18))
    print("balance of token at sender= %s"%(balance_sender/1e18))
